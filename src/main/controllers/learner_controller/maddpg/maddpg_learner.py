...
from src.main.controllers.learner_controller.utils import (
    save_pred_prey_losses,
    setup_pred_prey_losses_files,
)
...
class MADDPGLearner(Learner):
    def __init__(
        ...
        The Learner trains two separate Actor-Critic models for Predators and Preys,
        following the MADDPG algorithm. By leveraging the provided ActorSenderController, sends
        the models to the Predator-Prey environments. It also uses a ReplayBufferController
        to batch data from the Replay Buffer.
        :param replay_buffer_controller: ReplayBufferController
        :param num_predators: Num predators inside each environment
        :param num_preys: Num preys inside each environment
        :param num_states: Num states
        :param root_project_path: Root path of the project
        :param actor_sender_controller: ActorSenderController
        """
    ...
    def __update_critics(self):
        ...
        save_pred_prey_losses(self.root_project_path, losses[0], losses[1])
        return state_batch
